...
const DELIVERY_INSTRUCTIONS_STORAGE_KEY = "@hysafe_delivery_instructions";
...
export const CartProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    const loadCart = async () => {
      try {
        ...
        setDeliveryInstructions((await AsyncStorage.getItem(DELIVERY_INSTRUCTIONS_STORAGE_KEY)) || "");
      } catch (error) {
  ...
  useEffect(() => {
    if (!isLoaded) return;
    AsyncStorage.setItem(DELIVERY_INSTRUCTIONS_STORAGE_KEY, deliveryInstructions).catch((error) => console.error("Error saving delivery instructions:", error));
  }, [deliveryInstructions, isLoaded]);

  // Instructions are scoped to the current cart/order and must not leak to a later order.
  useEffect(() => {
    if (isLoaded && items.length === 0 && deliveryInstructions) setDeliveryInstructions("");
  }, [items.length, deliveryInstructions, isLoaded]);
  ...
  const clearCart = async () => {
    ...
    try {
      await AsyncStorage.multiRemove([CART_STORAGE_KEY, DELIVERY_INSTRUCTIONS_STORAGE_KEY]);
    } catch (error) {
