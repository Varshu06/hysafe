...
export const getOrderById = async (id: string): Promise<Order> => {
  try {
    ...
    return {
      ...order,
      driverName: order.assignedStaffId?.name || order.assignedStaff?.name || (order as any).assignedStaff?.name,
      assignedStaff: order.assignedStaffId || order.assignedStaff || (order as any).assignedStaff,
    };
  } catch (error: any) {
