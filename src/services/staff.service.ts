...
export const getAssignedOrders = async (): Promise<any[]> => {
  try {
    ...
    return response.data.map((order: any) => ({
      ...order,
      paymentMethod: order.isRecurring
        ? (order.paymentMethod === 'cash' || order.paymentMethod === 'shop' ? order.paymentMethod : order.recurringBillId?.preferredPaymentMethod || order.paymentMethod)
        : order.paymentMethod,
      id: order._id,
      customer:
