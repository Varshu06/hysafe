...
export const DeliveryInstructionsSheet = ({ visible, onClose, onSave, initial }: DeliveryInstructionsSheetProps) => {
  ...
  // Keep the sheet in sync with Checkout when it is opened again or edited.
  useEffect(() => {
    if (!visible) return;
    const labels = (initial || '').split(',').map((value) => value.trim()).filter(Boolean);
    setSelected(options.filter((option) => labels.includes(option.label)).map((option) => option.id));
  }, [visible, initial]);
            ...
            <View style={styles.sheet}>
              ...
              <View style={styles.actions}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    const value = options.filter((option) => selected.includes(option.id)).map((option) => option.label).join(', ');
                    onSave && onSave(value);
                    onClose();
                  }}
              >
              ...
              {selected.length > 0 ? <TouchableOpacity style={styles.clearButton} onPress={() => { onSave && onSave(''); onClose(); }}><Text style={styles.clearText}>Clear</Text></TouchableOpacity> : null}
              </View>
            </View>
          </TouchableWithoutFeedback>
        </View>
...
const styles = StyleSheet.create({
  ...
  clearText: { color: COLORS.textLight, fontWeight: '600' },
});
