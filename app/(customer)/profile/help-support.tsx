...
import { BUSINESS_ADDRESS, BUSINESS_EMAIL, BUSINESS_PHONE, COLORS, SERVICE_LOCATION_ADDRESS } from "../../../src/utils/constants";
  ...
  return <View style={styles.container}>
    ...
    <ScrollView contentContainerStyle={styles.content}>
      <View style={styles.card}>
        ...
        <TouchableOpacity accessibilityRole="link" onPress={() => openContactLink(`tel:${BUSINESS_PHONE}`)}>
          <Text style={styles.contactLink}>Phone: {BUSINESS_PHONE}</Text>
        </TouchableOpacity>
        <TouchableOpacity accessibilityRole="link" onPress={() => openContactLink(`mailto:${BUSINESS_EMAIL}`)}>
          <Text style={styles.contactLink}>Email: {BUSINESS_EMAIL}</Text>
        </TouchableOpacity>
        <Text style={styles.addressLabel}>Business address</Text>
        <Text style={styles.body}>{BUSINESS_ADDRESS}</Text>
        <Text style={styles.addressLabel}>Service / delivery location</Text>
        <Text style={styles.body}>{SERVICE_LOCATION_ADDRESS}</Text>
...
const styles = StyleSheet.create({ container:{flex:1,backgroundColor:COLORS.accent}, header:{flexDirection:"row",alignItems:"center",justifyContent:"center",paddingHorizontal:20,paddingBottom:16,borderBottomWidth:1,borderBottomColor:"#E2E8F0",position:"relative"}, back:{padding:16,top:16,position:"absolute",left:20},headerTitle:{fontSize:18,fontWeight:"bold",color:COLORS.text},placeholder:{width:40},content:{padding:20,gap:14},card:{backgroundColor:COLORS.secondary,borderRadius:16,padding:16,borderWidth:1,borderColor:"#E2E8F0"},sectionTitle:{fontSize:16,fontWeight:"bold",color:COLORS.text,marginBottom:8},body:{fontSize:13,lineHeight:19,color:COLORS.textLight,marginTop:4},contactLink:{fontSize:14,lineHeight:20,color:COLORS.primary,marginTop:5},addressLabel:{fontSize:13,fontWeight:"700",color:COLORS.text,marginTop:12},faq:{paddingVertical:10,borderTopWidth:1,borderTopColor:"#E2E8F0"},question:{fontSize:14,fontWeight:"700",color:COLORS.text} });
