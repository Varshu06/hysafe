...
export default function StaffOrderDetailsScreen() {
  ...
  const handleStatusUpdate = async (next: string, codCollected = false, collectedPaymentMethod?: 'cash' | 'shop') => {
    if (!order) return;
    try {
      await updateDeliveryStatus(order._id || order.id, next, undefined, undefined, undefined, codCollected, collectedPaymentMethod);
    ...
    <View style={styles.container}>
      ...
      <DeliveryConfirmModal
        ...
        onConfirm={async ({ codCollected, transactionId, notes, collectedPaymentMethod }) => {
          ...
          await handleStatusUpdate("delivered", codCollected, collectedPaymentMethod);
