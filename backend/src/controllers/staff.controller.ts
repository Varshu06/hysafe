...
export const toggleStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (fcmToken) {
      ...
      console.log(`🔔 Updating FCM token for staff: ${fcmToken.substring(0, 20)}...`);
    }
    ...
    console.log(`✅ Staff ${staff.name} is now ${isOnline ? 'online' : 'offline'}${fcmToken ? ' (FCM token updated)' : ''}`);
    if (fcmToken && staff.fcmToken) {
      console.log(`🔔 Staff ${staff.name} FCM token saved: ${staff.fcmToken.substring(0, 20)}...`);
...
export const acceptOrder = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log(`✅ Order ${order._id} accepted by staff ${staffId}. Status: ${order.status}, AssignedStaffId: ${order.assignedStaffId}`);
...
export const rejectOrder = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log(`Staff ${req.user._id} rejected order ${id}. Reason: ${reason || 'No reason provided'}`);
...
export const updateOrderStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log(`✅ Order ${order._id} status updated from ${previousStatus} to ${status} by staff ${req.user._id}`);
...
async function updateInventoryOnOrderAccept(quantity: number) {
  try {
    ...
    if (inventory.availableStock >= quantity) {
      ...
      console.log(`📦 Inventory updated: Reserved ${quantity} cans. Available: ${inventory.availableStock}, Reserved: ${inventory.reservedStock}`);
    } else {
      console.warn(`⚠️ Insufficient stock: Requested ${quantity}, Available: ${inventory.availableStock}`);
    }
  } catch (error: any) {
...
async function updateInventoryOnOrderDelivered(quantity: number, previousStatus: string) {
  try {
    let inventory = await Inventory.findOne();
    if (!inventory) {
    ...
    if (previousStatus === 'accepted' || previousStatus === 'out_for_delivery') {
    ...
    console.log(`📦 Inventory updated on delivery: Delivered ${quantity} cans. Total: ${inventory.totalStock}, Available: ${inventory.availableStock}, Reserved: ${inventory.reservedStock}`);
    
    // Check for low stock
    if (inventory.availableStock < inventory.lowStockThreshold) {
      console.warn(`⚠️ LOW STOCK ALERT: Available stock (${inventory.availableStock}) is below threshold (${inventory.lowStockThreshold})`);
    }
  } catch (error: any) {
...
async function updateInventoryOnOrderCancel(quantity: number, previousStatus: string) {
  try {
...
async function updateDriverInventory(staffId: mongoose.Types.ObjectId, quantity: number, action: 'pickup' | 'deliver' | 'return') {
  try {
    ...
    if (action === 'pickup') {
      ...
      console.log(`🚚 Driver ${staff.name} picked up ${quantity} cans. Total cans in hand: ${staff.cansInHand}`);
    } else if (action === 'deliver') {
      // Driver delivers cans to customer
      if (staff.cansInHand >= quantity) {
        staff.cansInHand = staff.cansInHand - quantity;
        console.log(`✅ Driver ${staff.name} delivered ${quantity} cans. Remaining cans in hand: ${staff.cansInHand}`);
      } else {
        console.warn(`⚠️ Driver ${staff.name} tried to deliver ${quantity} cans but only has ${staff.cansInHand} cans in hand`);
      ...
      if (staff.cansInHand >= quantity) {
        staff.cansInHand = staff.cansInHand - quantity;
        console.log(`🔄 Driver ${staff.name} returned ${quantity} cans. Remaining cans in hand: ${staff.cansInHand}`);
      }
    }
